# adguard-home-agent — Concept Overview
...
| **Action Execution Pipeline** | ✅ Built-in | Dry-run / commit / rollback phases (CONCEPT:ORCH-1.4) |
| **Resource Scheduling** | ✅ Built-in | Priority queuing + preemption limits (CONCEPT:OS-5.2) |
| **Session Concurrency** | ✅ Built-in | Enqueue/reject/interrupt/rollback (CONCEPT:OS-5.1) |
...
```
adguard-home-agent/
├── adguard_home_agent/        # Source code
│   ├── __init__.py
...
```json
{
  "mcpServers": {
    "adguard-home-agent": {
      "command": "uv",
      "args": ["run", "--with", "adguard-home-agent", "adguard-mcp"],
      "env": {}
    }
  }
}
...
```bash
adguard-mcp --transport streamable-http --port 8001
```
